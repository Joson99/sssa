...
// Function to collapse the user-dropdown
function collapseMenuDropdown() {
    const userMenubar = document.querySelector('.user-menubar');
    userMenubar.classList.remove('user-menubar-expanded');
    userMenubar.style.height = '40px';
    setTimeout(() => {
        userMenubar.classList.remove('visible'); // Remove visible class after animation
    }, 300); // Adjust the timeout to match the transition duration
}

// Function to toggle the expanded class on the user-dropdown
function toggleMenuDropdown() {
    const userMenubar = document.querySelector('.user-menubar');
    userMenubar.classList.toggle('user-menubar-expanded');
    
    if (userMenubar.classList.contains('user-menubar-expanded')) {
        const height = userMenubar.scrollHeight;
        userMenubar.style.height = height + 'px';
        setTimeout(() => {
            userMenubar.classList.add('visible'); // Add visible class after animation
        }, 300); // Adjust the timeout to match the transition duration
    } else {
        collapseMenuDropdown();
    }
}

// Add a click event listener to the logged-in user element
const sideHamburger = document.querySelector('.side-hamburger');
sideHamburger.addEventListener('click', toggleMenuDropdown);

// Add a click event listener to the document to collapse the user-dropdown when clicking outside
document.addEventListener('click', function (event) {
    const userMenubar = document.querySelector('.user-menubar');
    if (event.target !== sideHamburger && !userMenubar.contains(event.target)) {
        collapseMenuDropdown();
    }
});
